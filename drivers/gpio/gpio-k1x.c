...
{
	struct k1x_gpio_chip *k1x_chip = container_of(chip, struct k1x_gpio_chip, chip);
...
{
	struct k1x_gpio_chip *k1x_chip = container_of(chip, struct k1x_gpio_chip, chip);
	struct k1x_gpio_bank *bank = &k1x_chip->banks[k1x_gpio_to_bank_idx(offset)];
...
static int k1x_gpio_direction_output(struct gpio_chip *chip, unsigned int offset, int value)
{
...
static int k1x_gpio_irq_type(struct irq_data *d, unsigned int type)
{
	...
	struct k1x_gpio_bank *bank = &k1x_chip->banks[k1x_gpio_to_bank_idx(gpio)];
...
static int k1x_irq_domain_map(struct irq_domain *d, unsigned int irq, irq_hw_number_t hw)
{
	irq_set_chip_and_handler(irq, &k1x_muxed_gpio_chip, handle_edge_irq);
...
static int k1x_gpio_probe_dt(struct platform_device *pdev, struct k1x_gpio_chip *k1x_chip)
{
	u32 offset;
	int i, nbank, ret;
...
static int k1x_gpio_probe(struct platform_device *pdev)
{
	...
	domain = irq_domain_add_linear(np, k1x_chip->ngpio, &k1x_gpio_irq_domain_ops, k1x_chip);
	if (!domain)
		return -EINVAL;
	...
	if (devm_request_irq(&pdev->dev, irq, k1x_gpio_demux_handler, 0,
