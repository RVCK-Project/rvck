...
static struct phy *k3_comb_phy_xlate(struct device *dev, struct of_phandle_args *args)
{
...
static int k3_comb_phy_probe(struct platform_device *pdev)
{
	...
	{
		struct device_node *apbs_np;

		apbs_np = of_parse_phandle(node, "spacemit,apb-spare", 0);
		if (!apbs_np)
			return dev_err_probe(dev, -ENODEV,
					     "Failed to find APB SPARE syscon");
		phy->apb_spare = device_node_to_regmap(apbs_np);
		of_node_put(apbs_np);
	}
