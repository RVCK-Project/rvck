...
static int virtio_vdmabuf_dmabuf_vmap(struct dma_buf *dmabuf, struct iosys_map *map)
{
	struct virtio_vdmabuf_buf *exp_buf = dmabuf->priv;
	int heap_type = exp_buf->heap_type;
	struct page **pages;
	...
	case VIRTIO_VDMABUF_HEAP_TYPE_CARVEOUT:
		...
		map->vaddr_iomem = ioremap(exp_buf->bp[0].addr,
			       exp_buf->bp[0].size);
		map->is_iomem = true;
		return 0;
...
static void virtio_vdmabuf_dmabuf_vunmap(struct dma_buf *dmabuf, struct iosys_map *map)
{
	struct virtio_vdmabuf_buf *exp_buf = dmabuf->priv;
	int heap_type = exp_buf->heap_type;
	...
	case VIRTIO_VDMABUF_HEAP_TYPE_SYSTEM:
	case VIRTIO_VDMABUF_HEAP_TYPE_SYSTEM_CONTIG:
		vm_unmap_ram(map->vaddr_iomem, exp_buf->bp_num);
		break;
