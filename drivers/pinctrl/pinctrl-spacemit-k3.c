					 ...
					 unsigned int *num_maps)
{
	...
	for_each_available_child_of_node_scoped(np, child) {
		...
		ret = spacemit_pctrl_check_power(pctldev, child, pinmuxs, npins, grpname);
		if (ret < 0)
			return ret;
				      ...
				      unsigned int num_configs)
{
	...
	spin = spacemit_get_pin(pctrl, group->pins[0]);
	if (spacemit_pinconf_generate_config(pctrl, spin, &pctrl->data->variant,
					     configs, num_configs, &value))
		return -EINVAL;

	for (i = 0; i < group->num_pins; i++)
		spacemit_pin_set_config(pctrl, group->pins[i], value);
