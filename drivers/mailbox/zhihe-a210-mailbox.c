...
#define ZHIHE_A210_MBOX_CHAN_NAME_SIZE	20
...
struct zhihe_a210_mbox_context {
	u32 intr_mask[ZHIHE_A210_MBOX_CHANS - 1];
};
...
static void zhihe_a210_mbox_txdb_tasklet(unsigned long data)
{
	struct zhihe_a210_mbox_con_priv *cp = (struct zhihe_a210_mbox_con_priv *)data;
...
static int zhihe_a210_mbox_startup(struct mbox_chan *chan)
{
	...
	if (cp->type == ZHIHE_A210_MBOX_TYPE_DB)
		...
		tasklet_init(&cp->txdb_tasklet, zhihe_a210_mbox_txdb_tasklet,
			     (unsigned long)cp);
...
static void zhihe_a210_mbox_shutdown(struct mbox_chan *chan)
{
	...
	/* clear the chan mask */
	mask_bit = zhihe_a210_mbox_chan_id_to_mapbit(cp);
	zhihe_a210_mbox_rmw(priv, ZHIHE_A210_MBOX_MASK, 0, BIT(mask_bit));
...
static int zhihe_a210_mbox_probe(struct platform_device *pdev)
{
	...
	if (priv->version == ZHIHE_A210_MBOX_V1) {
		for (i = 0; i < ZHIHE_A210_MBOX_CHANS; i++) {
			...
			snprintf(cp->irq_desc, sizeof(cp->irq_desc), "zhihe_a210_mbox_chan[%i]",
				 cp->icu_cpu_idx);
...
static int zhihe_a210_mbox_remove(struct platform_device *pdev)
{
	return 0;
}
...
static int __maybe_unused zhihe_a210_mbox_resume_noirq(struct device *dev)
{
	...
	for (i = 0 ; i < ZHIHE_A210_MBOX_CHANS; i++)
		iowrite32(ctx->intr_mask[i],
			  priv->local_icu[i] + ZHIHE_A210_MBOX_MASK);
...
static struct platform_driver zhihe_a210_mbox_driver = {
	...
	.remove	= zhihe_a210_mbox_remove,
	.driver	= {
		.name		= "zhihe_a210_mbox",
