...
static int ccu_ddr_freq_chg(struct ccu_common *common,
			    struct ccu_mux_config *mux, u8 level)
{
	u32 reg;
	u32 timeout;
	unsigned long flags;
	...
	writel(reg | (level << mux->shift) | common->fc,
				     ...
				     u32 *mux_val)
{
	...
	for (i = 0; i < common->num_parents; i++) {
		...
		if (!parent)
			continue;
		parent_rate = clk_get_rate(clk_hw_get_clk(parent,
					   common->name));
...
static int ccu_ddr_determine_rate(struct clk_hw *hw,
